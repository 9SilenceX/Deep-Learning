...
#checks if the front-door camera picture (image_path) is actually the person called "identity".
def verify(image_path, identity, database, model):
    ...
    #tf.reduce_sum(tf.square(tf.subtract(encoding,database[identity])),axis=-1)
    #dist = tf.square(tf.subtract(encoding,database[identity]))
